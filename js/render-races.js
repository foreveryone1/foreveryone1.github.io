class RenderRaces{static $getRenderedRace(a){const b=Renderer.get().setFirstSection(!0);return $$`
		...
		${Renderer.utils.getExcludedTr(a,"race")}
		${Renderer.utils.getNameTr(a,{pronouncePart:a.soundClip?RenderRaces._getPronunciationButton(a):"",page:UrlUtil.PG_RACES})}
		${a._isBaseRace?"":`<tr><td colspan="6"><b>Ability Scores:</b> ${(a.ability?Renderer.getAbilityData(a.ability):{asText:"None"}).asText}</td></tr>
		...
		${a._isBaseRace?`<tr class="text"><td colspan="6">${b.render({type:"entries",entries:a._baseRaceEntries},1)}</td></tr>`:`<tr class="text"><td colspan="6">${b.render({type:"entries",entries:a.entries},1)}</td></tr>`}
